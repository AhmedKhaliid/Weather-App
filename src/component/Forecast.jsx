...
const Forecast = () => {
    ...
    return (
      <>
      {isLoading && <div className='w-full h-screen bg-slate-100/[0.5] flex flex-col justify-center items-center'>
        <img src={loadingImg} alt='' className='w-20 h-20' />
        <p>Loading...</p>
        </div>
      }
      {error &&<div className='w-full h-screen bg-slate-100/[0.5] flex flex-col justify-center items-center'>
      <img src={loadingImg} alt='' className='w-20 h-20' />
        <p>An error has occurred: {error.message}</p>
        </div>}
      {!isLoading && !error && <div className='sm:flex-col md:flex-row md:flex sm:space-y-8 md:space-y-0 md:justify-between w-full h-full '>
           
        <WeatherView weather={weather} timeDate={timeDate} allWeekWeather={allWeekWeather} />
        
        <WeatherDetails isLoading={isLoading} inputCountry={inputCountry}  weather={weather} countries={countries} onChange={(event) =>{
