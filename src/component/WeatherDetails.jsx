...
const WeatherDetails = ({countries,onChange,weather,isLoading,inputCountry}) => {
    ...
    useEffect(() => {
        if (weather) {
            setSunriseSunsetObj({sunrise: 'Sunrise',sunriseTime: weather.sunrise, sunrisePic: sunrisePic,
             sunset: 'Sunset',sunsetTime:weather.sunset, sunsetPic: sunsetPic})

             setWeatherElementsObj({avgHum: 'Average Humidity' , humPercentage: weather.avgHum, 
            rain: 'Chance of rain', rainPercentage: weather.rain , sign: "%"})
        }
    },[weather])

    return <div className=" bg-countryInputBG w-576px h-full pt-10  m-0 flex flex-col space-x-1 font-normal ">
        <div className="flex items-center h-fit pl-3">
        <IoLocationOutline className='animate-pulse' size={30} />
        <input disabled={isLoading ? true : false} className=" w-60 h-9 text-center bg-gray-500 bg-opacity-20" placeholder={inputCountry} type="text" list="countries" onChange={onChange}/>
...
<SunriseSunsetInfo weatherDate={weather.date} sunriseSunsetObj={sunriseSunsetObj} sunrise={weather.sunrise} sunset={weather.sunset}/>
